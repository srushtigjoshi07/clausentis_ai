...
export default function DocumentVaultPage() {
  ...
  return (
    <div className="flex flex-col gap-6 max-w-7xl mx-auto pb-12 font-sans">
      ...
      {/* Header */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-[#E5E5E5] pb-7 pt-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-3 rounded-xl border border-[#E5E5E5] bg-white p-3 space-y-1 shadow-sm">
          <div className="px-3 py-2 text-[11px] font-semibold text-[#777777] uppercase tracking-wider">
            Vault Domains
          </div>
          {VAULT_CATEGORIES.map((cat) => {
            ...
            return (
              <button
                ...
                className={`flex items-center justify-between px-3 py-2 rounded-lg text-xs sm:text-sm transition-colors text-left w-full border ${
                  isSelected 
        ...
        <div className="lg:col-span-9 flex flex-col gap-5">
          ...
          <div className="rounded-xl border border-[#E5E5E5] bg-white p-6 shadow-sm">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pb-4 border-b border-[#E5E5E5]">
              <div>
                <h3 className="text-sm sm:text-base font-semibold text-[#111111] flex items-center gap-2">
            ...
            <div className="mt-4 grid gap-4 sm:grid-cols-2">
              <div>
                <label className="text-[11px] font-semibold text-[#777777] block mb-1.5 uppercase tracking-wider">
                  Document Type
                </label>
                <select
                  value={uploadCategory}
                  onChange={(e) => setUploadCategory(e.target.value)}
                  className="w-full h-9 px-3 rounded-md bg-[#F7F7F7] border border-[#E5E5E5] text-xs sm:text-sm text-[#111111] focus:outline-none focus:border-[#111111]"
                >
            ...
            <div className="mt-5 flex justify-end">
              <Button
                ...
                size="sm"
                className="bg-[#111111] hover:bg-[#222222] text-white font-medium text-xs sm:text-sm h-9 px-5 disabled:opacity-50"
              >
                {isUploading ? (
                  <>
          ...
          <div className="rounded-xl border border-[#E5E5E5] bg-white shadow-sm overflow-hidden">
            <div className="p-4 border-b border-[#E5E5E5] bg-[#F7F7F7] flex flex-col sm:flex-row items-center justify-between gap-3">
              <div className="relative w-full sm:max-w-xs">
                <Search className="absolute left-3 top-2.5 h-4 w-4 text-[#777777] stroke-[1.5]" />
                <input
                  type="search"
                  placeholder="Search stored company documents..."
                  value={searchQuery}
                  onChange={(e) => setSearchQuery(e.target.value)}
                  className="w-full h-9 pl-9 pr-3 rounded-md bg-white border border-[#E5E5E5] text-xs sm:text-sm text-[#111111] placeholder:text-[#777777] focus:outline-none focus:border-[#111111] transition-colors"
                />
              </div>

              <div className="flex items-center gap-1 bg-white p-1 rounded-lg border border-[#E5E5E5] text-xs shrink-0">
                <button
            ...
            {loading ? (
              ...
              <div className="p-12 text-center flex flex-col items-center justify-center">
                ...
                <h4 className="font-semibold text-base text-[#111111]">Your Document Vault is empty</h4>
                <p className="text-sm text-[#555555] mt-1 max-w-sm leading-relaxed">
                  No documents found in this category. Upload company files above to automatically evaluate eligibility during tender intake.
                </p>
              </div>
            ) : viewMode === 'cards' ? (
              <div className="p-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredDocs.map((doc) => (
                  <div
                    key={doc.id}
                    className="p-4 rounded-xl border border-[#E5E5E5] bg-white shadow-xs hover:border-[#111111] transition-colors flex flex-col justify-between"
                  >
                    <div>
                      <div className="flex items-center justify-between mb-3">
                        <div className="h-8 w-8 rounded-lg bg-[#F7F7F7] border border-[#E5E5E5] flex items-center justify-center text-[#111111]">
                      ...
                      <h4 className="text-sm font-semibold text-[#111111] truncate" title={doc.file_name}>
                        {doc.file_name}
                      </h4>
                    ...
                    <div className="mt-4 pt-3 border-t border-[#E5E5E5] flex items-center justify-between">
                      ...
                      <button
                        ...
                        className="text-xs text-[#777777] hover:text-[#111111] transition-colors ml-auto"
                      >
                    ...
                    {filteredDocs.map((doc) => {
                      ...
                      return (
                        <tr key={doc.id} className="hover:bg-[#F7F7F7] transition-colors">
                          <td className="px-5 py-3.5 font-normal text-[#111111] max-w-xs truncate flex items-center gap-2">
                            <FileText className="h-4 w-4 text-[#777777] shrink-0 stroke-[1.5]" />
                            <span className="truncate font-medium">{doc.file_name}</span>
                          </td>
                          <td className="px-5 py-3.5 text-[#555555] capitalize">
                            {(doc.document_type || 'other').replace('_', ' ')}
                          </td>
                          <td className="px-5 py-3.5 text-[#777777] font-mono">
                            {formatDate(doc.created_at)}
                          </td>
                            ...
                            <div className="flex items-center justify-end gap-2">
                              {doc.file_path && (
                                <Button
                                  ...
                                  onClick={() => handleDownload(doc.file_path)}
                                  title="Download / View"
                                  className="h-8 w-8 text-[#555555] hover:text-[#111111] hover:bg-[#E5E5E5]"
                                >
                              ...
                              <Button
                                ...
                                disabled={isDeleting}
                                title="Delete"
                                className="h-8 w-8 text-[#777777] hover:text-[#111111] hover:bg-[#E5E5E5]"
                              >
                                {isDeleting ? (
