...
export function BidderSidebarContent({ pathname, onClose }: { pathname: string; onClose?: () => void }) {
  return (
    <>
      {/* Brand Header */}
      <div className="flex h-16 items-center border-b border-border px-6 justify-between">
        <Link href="/bidder/dashboard" className="flex items-center text-foreground" onClick={onClose}>
      ...
      <div className="flex-1 overflow-y-auto py-4 px-3">
        ...
        <nav className="grid gap-1">
          {routes.map((route) => {
            const isActive = pathname === route.path || (route.path !== '/bidder/dashboard' && pathname.startsWith(route.path));
...
export function BidderSidebar() {
  ...
  return (
    <aside className="hidden md:flex h-full w-64 flex-col border-r border-[#E5E5E5] bg-white z-20 select-none shrink-0">
      <BidderSidebarContent pathname={pathname} />
    </aside>
  );
}
